...
export const useSupabaseStorage = () => {
  const [bucketExists, setBucketExists] = useState<boolean | null>(true); // Always assume bucket exists
  const [uploading, setUploading] = useState(false);
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([]);
  const [error, setError] = useState<string | null>(null);

  // Simplified bucket check - just set to true
        ...
        for (const file of files) {
          ...
          if (uploadError) {
            ...
            if (uploadError.message.includes("Bucket not found")) {
              throw new Error(
                `Bucket '${BUCKET_NAME}' not found. Please create the '${BUCKET_NAME}' bucket in your Supabase Storage dashboard.`
              );
            } else if (
              uploadError.message.includes("The resource was not found")
            ) {
              throw new Error(
                `Storage bucket '${BUCKET_NAME}' doesn't exist. Please create it in your Supabase dashboard.`
              );
            } else if (
              ...
              throw new Error(
                `Permission denied. Please set up storage policies for the '${BUCKET_NAME}' bucket.`
              );
            } else {
              throw new Error(
  ...
  const deleteFile = useCallback(async (fileToDelete: UploadedFile) => {
    try {
      ...
      setUploadedFiles((prev) =>
        prev.filter((file) => file.url !== fileToDelete.url)
      );
    } catch (err) {
