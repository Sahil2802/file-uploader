import { ALLOWED_FILE_TYPES, ALLOWED_EXTENSIONS, type FileValidationResult } from '../types/file-upload'
...
export const validateFiles = (fileArray: File[]): FileValidationResult => {
  ...
  for (const file of fileArray) {
    ...
    if (isValidType || hasValidExtension) {
      ...
      errors.push(`${file.name}: File type not allowed. Only PDF, DOCX, and PNG files are accepted.`)
      invalidFiles.push(file)
    }
  }
